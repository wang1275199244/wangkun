...
<body>
    <div>
        ...
        <h4>选择简历</h4>
        <%
            List<Resume> list = (List<Resume>) session.getAttribute("resumeList");
            pageContext.setAttribute("list",list);
        %>
        <div>
                <c:if test="${list != null && fn:length(list) != 0}">
                    <c:forEach items="${list}" var="resume" varStatus="i">
                        <form action="toApplicationJob" method="post">
                            <table>
                                <tr>
                                    <th>姓名</th>
                                    <th>性别</th>
        ...
        <div id="page">第
            ...
            <c:if test="${list2 != null && fn:length(list2) != 0}">
                <c:forEach items="${list2}" varStatus="i">
                    <a href="toSelectResume?currentPage=${list2[i.index]}">${list2[i.index]}</a>
                </c:forEach>
            </c:if>
